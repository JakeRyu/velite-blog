...
1. `/getAllProducts`
    ...
    - **Problem**: The name includes a verb (get), which should be implied by the HTTP method (GET). It also suggests a specific action rather than a resource.
    - **Confusion**: What if we have an endpoint like `/getProductDetails`? The verb-based naming can easily become inconsistent and lead to confusion.
...
1. **Lowercase Letters**: Keep your URIs in lowercase to avoid confusion.
1. **Hyphens over Underscores**: Use hyphens for readability (`/user-orders` is clearer than `/user_orders`).
...
- API Documentation Tools: Tools like Swagger or Postman can help you maintain and document your API.
- Style Guides: Check out comprehensive guides from Google and Microsoft for more detailed conventions.
